...
public final class CaltechEval {
    ...
    private void init() {
        ...
        this.Operators[19] = "log";
        this.OperatorsArgs[19] = 2;
        this.OperatorsLeftRight[19] = false;
        this.OperatorsPrecedence[19] = 11;
    }
    ...
    private int getOperator(String op) {
        for (int i = 0; i < this.NOPERATORS; ++i) {
            if (!op.equals(this.Operators[i]) || op.length() != this.Operators[i].length()) continue;
            return i;
        }
        return -1;
    }
    ...
    private Vector<String> toTokens(String s) {
        Vector<String> tokenStrings = new Vector<String>(1);
        Object operand = "";
        Object operator = "";
        int nd = 0;
        int nt = 0;
        ...
        while (i < s.length()) {
            ...
            if (nt > 0 && ((String)operator).equals("atan") && c == '2') {
            ...
            if (this.inOperand(c)) {
                operand = (String)operand + s.charAt(i);
                ++nd;
                if (nt > 0) {
                    tokenStrings.addElement((String)operator);
                }
                nt = 0;
                operator = "";
                ++i;
            ...
            if (c == 'e' && nd > 0 && i < s.length() - 2 && (cnext == '+' || cnext == '-' || this.inOperand(cnext))) {
                operand = (String)operand + c;
                operand = (String)operand + cnext;
                nd += 2;
                nt = 0;
                i += 2;
                continue;
            ...
            if (c == '(' || c == ')') {
                if (nd > 0) {
                    tokenStrings.addElement((String)operand);
                }
                nd = 0;
                operand = "";
                if (nt > 0) {
            ...
            nd = 0;
            operand = "";
            if (((String)(operator = (String)operator + c)).equals("+") || ((String)operator).equals("-") || ((String)operator).equals("*") || ((String)operator).equals("/") || ((String)operator).equals(",")) {
                tokenStrings.addElement((String)operator);
                nt = 0;
                operator = "";
            } else {
    ...
    private double eval(String s) {
        Vector<String> tokens = this.toTokens(s);
        while (tokens.size() > 1) {
    ...
    private Vector<String> reduceTokens(Vector<String> tokens) {
        ...
        block0: while (tokens.contains("(")) {
            int ib = tokens.indexOf("(");
            Vector<String> bracketTokens = new Vector<String>();
            int brackets = 1;
        ...
        while (tokens.size() > 1) {
            ...
            for (it = 0; it < tokens.size(); ++it) {
                st = tokens.elementAt(it);
                iop = this.getOperator(st);
                if (iop == -1 || this.OperatorsPrecedence[iop] < maxprec) continue;
            ...
            value = this.doOp(st, rightValue1, rightValue2);
            stright = String.valueOf(value);
